...
char *user = "ngombe_watch";
char *password = "w@tcH_ng0m63"; /*end server details*/

char *database = "ngombe_watch";//if changed here, change also on the server"
...
int main()
{
    ...
    int iteration_sleep = 2; //the number of seconds to sleep in the middle of iterationsstruct test_struct
    struct timeval timeout;
    timeout.tv_sec = 60; //the timout for receiving data from client before closing connection i seconds
    timeout.tv_usec = 0;
...
int storeToDB(char * ax, char *ay, char *az,char *tm, char *dt, char *lt, char *ln, char *al, char *sp, char *cs, char *temp,char *battery, char *battVolt)
{
    ...
    sprintf(command, "INSERT INTO %s(ax,ay,az,tm,dt,lt,ln,al,sp,cs,temp,battery,battVolt) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",table_name, ax, ay, az,tm, dt, lt, ln, al, sp, cs, temp,battery, battVolt);
