...
struct conf{
  ...
  char *  phn1;//phone number 1
  char *  phn2;//phone number 2
};
...
void resetErrs(char *type)
{
  ...
  if(!SD.isFile(config_file)){
    ...
    obj.phn1  = SD.catln(config_file,2,1);//third index is for the phone number1
    obj.phn2  = SD.catln(config_file,3,1);//third index is for the phone number1
    ...
    if(SD.del(config_file)){
      ...
      if(SD.create(config_file)){
        ...
        if(SD.writeSD(config_file,newVal,0)){
        }//the incremented value of the number of retries

        memset(newVal,0,sizeof(newVal));
        sprintf(newVal,"%d",obj.tcpX);

        if(SD.appendln(config_file,newVal)){
        }//the original max number of retries before sending an sms 

        if(SD.appendln(config_file,obj.phn1)){
        }//the original phone number.

        if(SD.appendln(config_file,obj.phn2)){
        }//the original phone number.
  ...
  if (millis() - stime > GPRS_CONFIG_TIMEOUT){
    ...
    if(!SD.isFile(config_file)){
      ...
      if(obj.tcpR % obj.tcpX == 0){
        ...
        GPRS_Pro.sendSMS(message,obj.phn1);// USB.println("SMS Sent OK"); // * should be replaced by the desired tlfn number
        GPRS_Pro.sendSMS(message,obj.phn2);// USB.println("SMS Sent OK"); // * should be replaced by the desired tlfn number
      }
      ...
      if(SD.del(config_file)){
        ...
        if(SD.create(config_file)){
          ...
          if(SD.appendln(config_file,obj.phn1)){
          }//the original phone number1

          if(SD.appendln(config_file,obj.phn2)){
          }//the original phone number2
